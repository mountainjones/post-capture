...
for filename in os.listdir(directory):
    if filename.endswith(".JPG"):
        ...
        # Calculate the edges using Laplacian edge detection
        bw = cv2.imread(fName, 0)

        bw_scaled = cv2.resize(bw, (2048, 1365))
        laplacian = cv2.Laplacian(bw_scaled, cv2.CV_64F).var()
        ...
        if int(laplacian) > 100:
            vHTML = vHTML + '<td style="font-family: Courier New,Courier,monospace;">'+ str(round(laplacian,2)) +'</td>'
        else:
            vHTML = vHTML + '<td style="font-family: Courier New,Courier,monospace; color: red; font-weight: bold;">' + str(round(laplacian, 2)) + '</td>'
